...
| P0 | READY | unassigned | T3 | C10, D4 | `scripts/main.gd` | Route New Game through the creator. **Already true** — `_new_game()` enters `State.CREATOR`, `_finish_new_game()` (on `person_confirmed`) starts play. Leaving as READY rather than DONE since nobody has done the "Confirming creator reaches play; existing Continue remains safe" acceptance check as its own explicit, reviewed pass — the regression fix above tests the mechanism but wasn't scoped as a P0 review. Cheap for the next agent to just confirm and close. |
