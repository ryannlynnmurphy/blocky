...
| B1 | IN PROGRESS | Claude subagent | T2 | B0 | new city scene, terrain/roads/collision | Build the city-block scene. |
| B2 | IN PROGRESS | Claude subagent | T1 | B0 | asset manifest | Audit/import the approved (non-weapon, non-police) subset of `blocky/city/` assets. Running alongside B1 in the same subagent since both are small and share context. |
